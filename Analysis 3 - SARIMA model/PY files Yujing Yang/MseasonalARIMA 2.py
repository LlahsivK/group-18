...
def data_process():
    cl51_Merriwa = ['0212_Merriwa', '0213_Merriwa', '0214_Merriwa', '0215_Merriwa',
                     '0216_Merriwa', '0217_Merriwa', '0218_Merriwa']
...
def plotSARIMA(series, model, n_steps, real_data):
    ...
    plt.title("IOA: {0:.5f}".format(ioa))
    #     plt.axvspan(data.index[-1], forecast.index[-1], alpha=0.5, color='lightgrey')
    #     plt.plot(real_data, label="actual")

    plt.legend()
    plt.grid(True)
    plt.savefig('./Mprediction.jpg')
...
def ARIMA_model(data):
    ...
    plt.title("cl51 Merriwa train")
    plt.savefig('./M_bl_height.jpg')
    plt.show()
    ...
    plt.savefig('./M_seasonaltest.jpg')
